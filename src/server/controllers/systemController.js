...
const getProjectById = async (_req, res) => {
    try {
        ...
        await mongo_cache_1.mongoCacheSystem.load();
        const value = await mongo_cache_1.mongoCacheSystem.get(id);
        if (value) {
...
const getAllProjects = async (_req, res) => {
    try {
        await mongo_cache_1.mongoCacheSystem.load();
        const value = await mongo_cache_1.mongoCacheSystem.getAll();
